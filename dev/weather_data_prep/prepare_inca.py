...
def load_inca_and_compute_daily_agg(path: str, parameter: str, period: str):
    ...
    if period == '2017':
        ...
        if parameter == 'mslp' or parameter == 'relative_humidity' or parameter == 'temp':
        ...
        if paramater == 'mslp':
            encoding = {parameter: {'dtype': 'int16',
            'scale_factor': 0.1, 
            'zlib': True,'complevel': 9}}
        else:         
            encoding = {parameter: {'dtype': 'int16',
            'scale_factor': 0.1, '_FillValue': -9999,
            'zlib': True,'complevel': 9}}

        ds_agg.to_netcdf(path+ '/' + parameter + '/' +
        "INCA_2017090100_20171231_23_" + parameter +".nc",
        engine="h5netcdf")
        ...
        for year in ['2018', '2019', '2020', '2021', '2022']: 
            ...
            if period == 'first_half':
                ds_agg.to_netcdf(path+ '/' + parameter + '/' +
                "INCA_"+ year+'010100'+ "_"+ year+'063023'+ "_" + parameter +".nc",
                engine="h5netcdf")
            else: 
                ds_agg.to_netcdf(path+ '/' + parameter + '/' +
                "INCA_"+ year+'070100'+ "_"+ year+'123123'+ "_" + parameter +".nc",
                engine="h5netcdf")
