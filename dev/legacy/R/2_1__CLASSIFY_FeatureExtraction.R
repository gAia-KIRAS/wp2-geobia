...
sf_segStats <- sf_segStats %>% dplyr::rename(
  .data = ., dtm = dtm..MEAN.,
  open = open..MEAN.,
  slp = slp..MEAN.,
  cv_max7 = cv_max7..ME,
...
sf_segStatsSM <- sf_segStatsSM %>% dplyr::select(names(sf_segStatsSM)[names(sf_segStatsSM) %in%
  c(
    "P", "P.A", "P.sqrt.A", "P.sqrt.A.", "Dmax", "Dmax.A", "Dmax.sqrt.A", "Shape.Index",
...
sf_segStatsSM <- sf_segStatsSM %>% dplyr::mutate(
  A_cHull = sf_segStatsSM_cHull %>% sf::st_area(x = .) %>% as.numeric(.),
...
sf_segStats <- sf_segStats %>% dplyr::mutate(
  Flow = ((atan2(flSin, flCos) * (-180) / pi) + 90) %% 360,
...
# names(sf_segStats)
feature_col_names <- c(
  "slp", "open", "cv_max7", "cv_min7", "cv_prf7", "cv_pln7",
...
# ... merge data
sf_segStats <- sf_segStats %>%
  merge(x = ., y = contiguity_Queen, by = "ID", all.x = TRUE) %>%
...
sf_segStatsFin <- sf_segStatsFin %>% dplyr::mutate(
  X = coords_segStats[, 1],
  Y = coords_segStats[, 2]
)
...
# ... get TRUE object IDs
ID.TRUE.Scrp <- c(
  df_Pts2Inv %>% dplyr::filter(TRUE_Scrp == 1) %>% dplyr::select(ID),
...
ID.TRUE.Bdy <- c(
  df_Pts2Inv %>% dplyr::filter(TRUE_Bdy == 1) %>% dplyr::select(ID),
...
saveRDS(
  object = sf_segStatsFin %>% sf::st_drop_geometry(.),
