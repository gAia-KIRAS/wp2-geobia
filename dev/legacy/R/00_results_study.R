...
plot(
  main = "inventory of validation area",
...
results_study$optim_HPFT %>%
  .[order(.$fscore_3, decreasing = T), ] %>%
  head(.)
...
results_study$optim_SegmScrp %>%
  .[order(.$Objective.Function, decreasing = T), ] %>%
  head(.)
...
f_score.selection <- results_study$optim_HPFT %>%
  .[order(.$fscore_3, decreasing = T), ] %>%
  .[1, ]
...
ggplot(results_study$optim_HPFT, aes(x = scale, y = threshold, z = fscore_3)) +
  ...
  directlabels::geom_dl(aes(label = ..level..),
    stat = "contour",
    method = list("top.pieces", cex = 0.7, colour = "black", fontfamily = "Arial"), breaks = breaks.contour
  ) + # top.pieces
...
Lslide::plotObjectiveFunction(results_study$optim_SegmScrp,
  selected.Scale = 0.075, selected.Scale.size = 4, selected.Scale.label = "selected local optimum",
...
Lslide::plotObjectiveFunction(results_study$optim_SegmBdy,
  selected.Scale = 3.25, selected.Scale.size = 4, selected.Scale.label = "selected local optimum",
  y.axis.label.left = "objective function values", y.axis.label.right = "normalized values"
) +
...
varTable <- data.frame(
  ...
  Name_full = c(
    "compactness", "convexity", "curvature, minimum", "curvature, minimum, flow contiguity", "curvature, minimum, Queen's contiguity",
  ...
  Name = c(
    "compactness", "convexity", "curvature, minimum", "curvature, minimum, \nflow contiguity", "curvature, minimum, \nQueen's contiguity",
...
ggplot2::ggplot(df_varImp, ggplot2::aes(x = Name, y = Kappa, fill = Type), colour = NA) +
  ...
  theme_bw() +
  theme(
    legend.position = "bottom", text = element_text(size = 10, face = "bold", family = "Arial"),
