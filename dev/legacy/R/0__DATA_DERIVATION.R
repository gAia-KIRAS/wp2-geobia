...
r_flow_Dinf_Area.log <- raster::calc(x = r_flow_Dinf_Area, fun = function(x){return(log(x))})
...
r_flow_Dinf_Area_log <- raster::calc(x = r_flow_Dinf_Area, fun = function(x){return(log(x))})
...
sin_flow_Dinf <- raster::calc(r_flow_Dinf, fun = function(x){return(sin(x))})
cos_flow_Dinf <- raster::calc(r_flow_Dinf, fun = function(x){return(cos(x))})
...
flow_Dinf_Rotated <- raster::calc(r_flow_Dinf, fun = function(x){return((((x * (-180)/pi) + 90) %% 360))})
...
GeoMorphTB::roughnessIndex(elevation = data_dtm, slope = "", aspect = "", tool = c("RI"), output.path = path_save,
                           size = 7, output = path_roughness.i15, return_weighted = FALSE,use.SAGA = TRUE, env = env.rsaga, quiet = FALSE)


# ... SkyViewFactor 
skyViewFactor(path.software = path_svf_software,  sky_view_factor = c(1, 16, 20, 0, "low"),
              path_save = path_save, quiet = FALSE)

r_SVF<- r.SVF <- raster::raster(file.path(path_save, "AOI_DGM_01m_SVF_R20_D16.tif"))
r_SVF<- raster::overlay(x = r_SVF, y = data_dtm, fun = function(x, y){ifelse(is.na(y), NA, x)})
...
# calculate texture
Lslide::textureFlow(grass.texture.flowDir = path_flow_Dinf_deg, grass.texture.input = "slope", grass.texture.method = grass.texture.method, grass.texture.name = grass.texture.name,
...
r_textFlowD_SD51 <- raster::calc(r_textFlowD_Var51, fun = function(x){return(sqrt(x))})
