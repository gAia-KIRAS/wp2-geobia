...
# ... performance
perf_train2valid <- mlr::performance(pred = pred_train2valid, measures = list(mlr::kappa, mlr::mmce, mlr::bac, mlr::ber, mlr::acc, 
...
valid_sf_Lslide_pred <- valid_segment %>% 
                            merge(x = ., by = "ID", all.y = TRUE,
                                  y = valid_df %>% dplyr::mutate(truth = pred_train2valid$data$truth,
...
valid_sp_Lslide_scarp <- Lslide::neighborGrowing(spdf = subset(valid_sp_Lslide_pred, valid_sp_Lslide_pred@data$response == 1), 
                                                 return.gUnaryUnionNeighbors = FALSE, return.input = TRUE)


# ... get landslide body for corresponding scarps (using growing classes)
valid_df_Lslide_class <- getGrowingClass(grownInput = valid_sp_Lslide_scarp, 
...
valid_df_Lslide_AddOn_IDScrp <- valid_sp_Lslide_scarp %>% {.@data$ID[.@data$response == 1 & .@data$prob_1 >= 0.8]} %>%
                                setdiff(x = ., y = valid_df_Lslide_class$ID[valid_df_Lslide_class$type == "Scarp_ID"])


valid_df_Lslide_AddOn_IDBody <- valid_sp_Lslide_pred %>% {.@data$ID[.@data$response == 2 & .@data$prob_2 >= 0.8]} %>%
...
# ... joining
valid_sp_Lslide_postpros_merge <- merge(x = valid_sf_Lslide_pred, 
                               y = valid_df_Lslide_class_postpros %>% dplyr::select("ID", "Lslide_cl"), 
                               by = "ID", all.x = TRUE) %>% 
                          as(., "Spatial")

# ... final growing
valid_sp_Lslide_postpros  <- Lslide::neighborGrowing(spdf = subset(valid_sp_Lslide_postpros_merge, !is.na(valid_sp_Lslide_postpros_merge@data$Lslide_cl)), 
