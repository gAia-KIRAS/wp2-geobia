                                 ...
                                 conv.thresh = 0.5, tol = 75, cores = 20)
{
  ...
  seg.bb.flow.Conv <- Lslide::getBoundingBox(spdf = subset(seg.sp, seg.sp[[col.name.conv]] < conv.thresh & !is.na(seg.sp[[col.name.fl]])), scale.factor = c(1.5, 1.3), cores = cores,
  ...
  seg.bb.flow.Centr <- Lslide::getBoundingBox(spdf = subset(seg.sp, seg.sp[[col.name.conv]] >= conv.thresh & !is.na(seg.sp[[col.name.fl]])), scale.factor = c(2, 1.3),
  ...
  seg.nbFlowDirTol <- Lslide::neighborDirection(spdf = seg.sp, col.name = col.name.fl, modus = "nb", 
                    ...
                    v.F = c(0), v.T = c(1), col.fit = NULL, v.fit = NULL, seed = 1234)
{
  ...
  if(do.sample)
  {
    ...
    {
      if(length(v.fit) != length(v.T)){stop("Length of TRUE items and TRUE sample items differ! \n")}
...
getGrowingClass <- function(grownInput, nb, orig_d, thresh, col.prob.body = "Body_Prb1", out.col1 = "NeighborGrowing", out.unique = TRUE)
{
  ...
  out <- lapply(1:length(unique(grownInput$NeighborGrowing)), function(x, grownInput, nb, orig_d, thresh){
    ...
    if(length(nb.tmp) == 0){return(data.frame(Scarp_ID = id.grownInput, Body_ID =  NA, NeighborGrowing = x))}
  ...
  if(out.unique)
  {
    ...
    for(i in id.tmp)
    {
      ...
      repeat{
        ...
        if(length(sel.id.tmp) == l.tmp){break} # break out when all is found
      }
...
rsaga.calcAccPixelBased <- function(r, inventory, env.rsaga = NULL, show.output.on.console = FALSE)
{
  tryCatch({
    ...
    if(!raster::compareRaster(r, inventory, stopiffalse = FALSE)){
      ...
      if(!raster::compareRaster(r, inventory)){stop("Rasters mismatch in their attributes (resolution, crs, etc ...)")}
    }
    ...
    if(length(r.val) == 1 && is.na(r.val)){stop("Raster contains only NAs")}
    ...
    if(!is.null(env.rsaga))
    {
      ...
      r <- raster::calc(x = r, fun = function(x){ifelse(x == 0, 0, 2)})
    ...
    df.stat$rndm_acc <- tryCatch({((as.numeric(df.stat$TN + df.stat$FP) * as.numeric(df.stat$TN + df.stat$FN)) + (as.numeric(df.stat$FN + df.stat$TP) * as.numeric(df.stat$FP + df.stat$TP)))/(as.numeric(sum(df.stat[1, 1:4])) * as.numeric(sum(df.stat[1, 1:4])))}, error = function(e){return(NA)})
