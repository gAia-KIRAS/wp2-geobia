...
#lc <- qread("wp2-geobia/dat/interim/misc_aoi/land_forest_cover.qs", nthreads = ncores) 
#stopifnot(identical(st_coordinates(dtm), st_coordinates(lc)))
#lc <- st_drop_geometry(lc)

# land cover NOE
print(glue("{Sys.time()} -- reading land cover features"))
lc <- qread("wp2-geobia/dat/interim/misc_aoi/land_cover.qs", nthreads = ncores) 
stopifnot(identical(st_coordinates(dtm), st_coordinates(lc)))
lc <- st_drop_geometry(lc)
...
ci <- qread("wp2-geobia/dat/interim/misc_aoi/climate_indices.qs", nthreads = ncores)
...
rd <- qread("wp2-geobia/dat/interim/misc_aoi/road_dist.qs", nthreads = ncores) %>%
...
out <- dtm %>%       
  ...
  #bind_cols(esa) %>%
  bind_cols(inv) %>%
  rename_all(~gsub("-", "_", .x)) %>%  # Rename all columns by replacing "-" with "_"
  rename_all(tolower) %>%  # Convert all column names to lowercase
  select(slide, everything())  # Select columns, including 'slide', and all other columns
...
res <- out %>%
  sfc_as_cols() %>%
  st_drop_geometry()
write_ipc_file(res, sink = "wp2-geobia/dat/processed/noe_10m.arrow", compression = "lz4")
