...
vars_sparta_winfore = ['api_p0.935_k7_yearpctl95', 'api_p0.935_k30_yearpctl95',
'SPEI30_yearpctl95', 'pci']
...
for filename in ['climdex_concat_indices.nc', 'misc_concat_spartacus_winfore_indices.nc']:
    if filename == 'climdex_concat_indices.nc':
        ds = xr.open_dataset(path_in / filename)
        ds_sel = ds.sel(variable = vars_climdex, time=slice(climate_normal_begin, climate_normal_end))
        ds_sel_q95 = ds_sel.quantile(0.95, dim='time')
        ds_sel_q95.rio.write_crs("EPSG:31287", inplace=True)
        ds_sel_q95_cut = ds_sel_q95.rio.clip(gdf_aoi.geometry.values, gdf_aoi.crs, drop=True, invert=False)
        ...
        ds_sel = ds.sel(variable = vars_sparta_winfore, time=slice(climate_normal_begin, climate_normal_end))
        ds_sel_mean = ds_sel.mean(dim='time')
        ds_sel_mean.rio.write_crs("EPSG:31287", inplace=True)
        ds_sel_mean_cut = ds_sel_mean.rio.clip(gdf_aoi.geometry.values, gdf_aoi.crs, drop=True, invert=False)
