...
# Land cover #ERROR
lc <- read_stars("wp2-geobia/dat/interim/misc_aoi/cadasterenv_noe.tif")
res_lc <- st_extract(x = lc, pts = grd, bilinear = FALSE) %>%
  rename(land_cover = "cadasterenv_noe.tif")
stopifnot(nrow(grd) == nrow(res_lc))
qsave(res_lc, "wp2-geobia/dat/interim/misc_aoi/land_cover.qs", nthreads = ncores)
