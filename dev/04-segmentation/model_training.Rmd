...
load("~/nfs_home/Git/gaia/dat/interim/segments/segments_ktn_subset1.RData")
...
confusionMatrix(testing$class, predict(rngFull, data=testing)$pred, positive="1")
```
...
set.seed(101)
rngFullCv <- train(class ~ ., training, method="ranger", 
...
confusionMatrix(testing$class,
                predict(rngFullCv$finalModel, data=testing)$pred, positive="1")
```
...
for (i in 1:nrep) {
    set.seed(42)
    rngFit2[[i]] <- train(.outcome ~ ., rngFit1[[i]]$trainingData,
                          method = "ranger", trControl=fitControl2,
                          tuneGrid = bestHParSet, num.threads = nthreads)
}
```
...
confusionMatrix(testing$class, rngMajVote, positive = "1")
```
